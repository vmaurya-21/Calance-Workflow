...
func (s *GitHubOAuthService) GetOrganizationRepositories(ctx context.Context, accessToken, orgName string) ([]GitHubRepository, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (s *GitHubOAuthService) GetUserRepositories(ctx context.Context, accessToken string) (map[string][]GitHubRepository, error) {
	...
	for _, org := range organizations {
		repos, err := s.GetOrganizationRepositories(ctx, accessToken, org.Login)
		if err != nil {
